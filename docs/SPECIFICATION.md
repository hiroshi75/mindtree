...
*   **画面構成:**
    ...
    *   **専用パネル (右側):**
        ...
        *   **プロンプト入力エリア:**
            ...
            *   プロンプトの永続化:
                *   各ノードごとに最後に使用したプロンプトを保存
                *   ノード選択時に保存されたプロンプトを自動的に読み込み
                *   初めて使用するノードの場合は「このノードのアイデアを膨らませてください」をデフォルト表示
                *   プロンプト変更時に自動的にデータベースに保存
        *   **生成数設定:**
            *   数値入力: 生成する子ノードの数を指定（1～20）
        *   **詳細設定 (折りたたみ可能):**
            *   ラベル: "詳細設定"
            *   **LLM選択:**
...
*   **作成者:** Gemini
*   **更新履歴:**
    *   2025年2月2日: LLMパネルのプロンプト永続化機能を追加
    *   2025年2月2日: LLMパネルの生成数設定を詳細設定から外し、1～20の範囲に制限
    *   2025年2月2日: 状態管理を改善し、DBの状態とUIの同期を最適化
